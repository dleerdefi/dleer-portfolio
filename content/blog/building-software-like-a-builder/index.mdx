...
I didn't learn to ship software in a CS lab. I learned it on muddy jobsites, helping engineer a **$548 million medical campus**. My job was to make sure the hidden systems–medical gas, reverse-osmosis lines, even pneumatic tubes–would mesh perfectly within the concrete and steel. On a project of that scale, you think in dependencies and sequencing because the only alternative is chaos.
...
I call it the **preconstruction mindset**: plan first with ruthless clarity, then execute in slices.

In the construction industry, the "preconstruction" phase is formally recognized as ["the backbone of any construction project"](https://www.letsbuild.com/blog/what-is-preconstruction-and-why-its-important). It's a defined discipline for ["risk mitigation, cost control, project planning, and quality assurance"](https://www.procore.com/library/preconstruction-software). It's the phase where all stakeholders align on the scope, budget, and schedule specifically to ["identify potential roadblocks"](https://www.autodesk.com/solutions/preconstruction-software) *before* a single foundation is poured.
...
This creates a powerful feedback loop. Construction's rigor provides the *discipline* modern software development desperately needs. Software's iteration (Agile) provides the *flexibility* complex engineering projects demand.

My "preconstruction mindset," therefore, is the synthesis of both. It's not a rigid, "waterfall" plan. It is a rigorous *but flexible* planning discipline designed to de-risk complexity. It's the pivot from *coder* to *project manager*, and it's the single most important skill for building with AI. It's the difference between "it works on my machine" and a clean, resilient deployment.

## The Siren Song of "Vibe Coding"

And then, AI came for the code.

Generative AI makes it dangerously tempting to adopt the *wrong* workflow. Andrej Karpathy, a founding member of OpenAI, gave this a name:
...
The *real* bottlenecks are clarifying requirements and reviewing code. Vibe coding doesn't solve these bottlenecks; it just moves them and makes them worse. The delay simply shifts elsewhere—when senior developers are overwhelmed reviewing AI-generated code that is architecturally unsound. Your "48 hours spamming 'pls fix'" *is* the new bottleneck.

This isn't just a process problem; it's a *skills* problem. The temptation is to use AI as a magic black box, which creates a [dangerous trifecta: developers produce more vulnerabilities, feel more confident, and fix fewer of them](https://cset.georgetown.edu/wp-content/uploads/CSET-Cybersecurity-Risks-of-AI-Generated-Code.pdf).

But this isn't a failure of AI; it's a failure of *method*. The real bottleneck isn't the AI, it's the human-in-the-loop failing to provide direction. The skills that made a good "coder" (like typing speed) are being automated. The skills that make a good *builder*–like planning, risk assessment, and review–are now more valuable than ever.

## The Cost: A Mountain of AI-Generated Debt

When you let "vibe coding" drive your project, the result is a mountain of AI-generated debt, created at a speed I've never witnessed before.

<Admonition type="error" title="Technical Debt Crisis">
**"I don't think I have ever seen so much technical debt being created in such a short period of time during my 35-year career in technology,"** –API evangelist Kin Lane.
</Admonition>

This isn't just a feeling. A [2024 GitClear analysis of 211 million lines of code](https://leaddev.com/technical-direction/how-ai-generated-code-accelerates-technical-debt) found staggering evidence:

* It tracked an **8-fold increase** in code duplication.
* It showed that ["code reuse is dying," with "copy-pasted lines exceed[ing] moved lines"](https://leaddev.com/technical-direction/how-ai-generated-code-accelerates-technical-debt) for the first time.

This is the direct result of vibe coding. A developer prompts for a feature. An AI, trained on public repos, provides a non-optimal solution that *works* in isolation. The developer pastes it in. Another developer, working on a similar feature, does the same.
Instead of one, clean, reusable module, the team now has eight duplicated, overly complex blocks of spaghetti code. *That* is the technical debt that will grind your project to a halt.

It's not just messy; it's a security time bomb.

<Admonition type="error" title="Security Risks">
* Research from Veracode shows that **45% of AI-generated code samples fail security tests**.
* A Stanford study confirms AI-generated code ["contains security bugs"](https://www.legitsecurity.com/aspm-knowledge-base/ai-code-generation-benefits-and-risks).
</Admonition>

The AI, trained on vast repositories of public code, is unconsciously reproducing known vulnerabilities and risks introducing the very SQL injection, XSS attacks, and data leaks that a human expert would know to prevent.

Worse, as AI agents become more autonomous, we face what Martin Fowler's colleague Korny Sietsma calls the ["Lethal Trifecta": **Sensitive Data + Untrusted Content + External Communication**](https://martinfowler.com/articles/agentic-ai-security.html). An agentic IDE given access to your codebase, your secrets, and the internet *is* this trifecta. An attacker can use "untrusted content"–like a prompt injection hidden in a file–to instruct the LLM to ["leak sensitive data"](https://martinfowler.com/articles/agentic-ai-security.html).
...
But this presents a new challenge. As my construction experience taught me, a good project manager *must* have a strong understanding of what they are managing. If you don't have a technical CS background, you will face a wall of "unknown unknowns".
...
This "LLM-as-reviewer" process is how you use AI to surface the very "unknowns" that would otherwise sink your project.

This is *not* a return to a 500-page waterfall document. Look at my spec template below. It's a simple markdown document. It includes "Work Packages (1–2 days each)"–those are *sprints*. My central rule–"When changes occur, *update the SPEC first*, then the code"–makes the spec a *living document*, not a rigid plan.

This is **Agile Specification-Driven Development**. It combines the continuous feedback and refinement of Agile with the "preconstruction risk mitigation" of a builder. It is the workflow that lets you build fast *and* build right.

## Phase 1: Preconstruction

Here is the antidote. Before you pick your tools, define your plan.
...
With your SPEC.md in the repo, you're ready for construction. This isn't a "fire and forget" prompt. It's a tight, disciplined loop.
