...
Construction has phases: sitework → foundations → framing → MEP → finishes.
...
### Step 3: Implement With Testing Gates

Submit the spec to your coding agent:

Press <Key>Cmd</Key> + <Key>K</Key> then type: "Ingest `docs/BLOG_V2_SPEC.md` and implement M1. Run build test after."
...
- ✅ **Step 1**: Wrote [BLOG_V2_SPEC.md](https://github.com/example/dleer-portfolio/blob/main/docs/BLOG_V2_SPEC.md)
- ✅ **Step 2**: Broke into M1–M7 with DoD per milestone
- 🔄 **Step 3**: Implementing M3 (Zen list + keyboard nav)
- ⏳ **Step 4**: Remaining: M4–M7
...
- **CI**: GitHub Actions (lint → type → build → deploy)
